...
response = WS.sendRequest(findTestObject('UpdateCalls/PatchUpdate'))
...
if (conditionStatusCode) {
	...
	if (conditionSchema) {
		KeywordUtil.logInfo('Response schema is correct.')
			...
			conditionResponseParam = WS.verifyElementPropertyValue(response, 'job', 'zion resident', FailureHandling.OPTIONAL)
			if(conditionResponseParam) {
				KeywordUtil.markPassed('Updated user job is correct.')
			}
			else {
				KeywordUtil.markFailedAndStop('Updated user job is incorrect.')
			}
